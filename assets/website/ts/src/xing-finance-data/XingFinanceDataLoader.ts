...
export default class XingFinanceDataLoader {
    ...
    constructor(marketCapClass: string, gifClass: string, performanceTextClass: string) {
        ...
        this.performanceTextElement = document.querySelector(
            performanceTextClass,
        ) as HTMLElement | null;
        if (
            marketCapElements.length !== 2 ||
    ...
    private displayXingMarketCap(jsonResponse: XingFinanceDataResponse): void {
        this.marketCapElement.textContent =
            '$ ' + jsonResponse.finance.marketCap.toLocaleString('en-EN');
        this.leftToBillionElement.textContent =
            '$ ' + (1000000000 - jsonResponse.finance.marketCap).toLocaleString('en-EN');
    }
    private displayGif(jsonResponse: XingFinanceDataResponse): void {
        const source = (this.gifPictureElement as HTMLElement).querySelector(
            'source',
        ) as HTMLSourceElement;
        const img = (this.gifPictureElement as HTMLElement).querySelector(
            'img',
        ) as HTMLImageElement;
    ...
    private displayPerformanceText(jsonResponse: XingFinanceDataResponse): void {
        const performanceTextSpanElement = (
            this.performanceTextElement as HTMLElement
        ).querySelector('span') as HTMLSpanElement;
        if (jsonResponse.finance.priceChange >= 0) {
            performanceTextSpanElement.textContent =
                '+' + jsonResponse.finance.priceChange.toLocaleString('en-EN') + ' % 🐵';
        }
        if (jsonResponse.finance.priceChange < 0) {
            (this.performanceTextElement as HTMLElement).classList.remove(
                'xing_information-right-container-price-change-positive-text',
            );
            (this.performanceTextElement as HTMLElement).classList.add(
                'xing_information-right-container-price-change-negative-text',
            );
            performanceTextSpanElement.textContent =
                jsonResponse.finance.priceChange.toLocaleString('en-EN') + ' % 🙈';
        }
