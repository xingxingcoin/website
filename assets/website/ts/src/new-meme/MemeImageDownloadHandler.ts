...
export default class MemeImageDownloadHandler {
    ...
    constructor(
        ...
        this.memeTextColorPicker = document.querySelector(
            memeColorPickerInputClass,
        ) as HTMLInputElement | null;
        this.memeTextSizeInput = document.querySelector(
            memeFontSizeNumberInputClass,
        ) as HTMLInputElement | null;
        if (
    ...
    private initEventListener(): void {
        (this.downloadButton as HTMLLabelElement).addEventListener('click', (): void => {
            const memeImageCanvas: HTMLCanvasElement | null = document.querySelector(
                '#meme-preview-container div canvas',
            );
            const memeText: HTMLParagraphElement | null = document.querySelector(
                '#meme-preview-container div p',
            );
            if (memeImageCanvas === null) {
                return;
            }
            let memeCanvas: HTMLCanvasElement | null = memeImageCanvas;
            if (memeText !== null) {
                memeCanvas = this.generateCanvasWithText(memeCanvas, memeText);
            }
     ...
     * @exception Error
     */
    private generateCanvasWithText(
        memeImageCanvas: HTMLCanvasElement,
        memeText: HTMLParagraphElement,
    ): HTMLCanvasElement {
        ...
        const memeCanvasContainer = document.querySelector(
        ...
        context.drawImage(
            ...
            0,
            0,
            memeCanvasContainerRect.width,
            memeCanvasContainerRect.height,
        );
        context.fillText(
            (memeText.textContent as string).toUpperCase(),
            memeTextPositionX,
            memeTextPositionY,
        );
        context.strokeText(
