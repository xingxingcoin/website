...
export default class MemeGeneratorImagesByMemeImageFilterLoader {
    ...
    constructor(
        ...
        if (
            ...
            throw new Error('Meme generator images by meme image filter are not loaded.');
        }
    }
    ...
    public load(): void {
        ...
        ajaxHttpClient.onreadystatechange = (): void => {
            if (ajaxHttpClient.readyState === 4 && ajaxHttpClient.status === 200) {
                ...
                const event = new Event('afterMemeImageFilterButtonClicked');
                (this.memeImageFilterButton as HTMLAnchorElement).dispatchEvent(event);
            }
        };
