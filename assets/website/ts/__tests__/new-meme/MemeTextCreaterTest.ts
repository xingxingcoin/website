...
jest.mock('interactjs', (): any => {
    return jest.fn((): any => {
        return {
            draggable: jest.fn().mockReturnThis().mockImplementation(({listeners}: any): void => {
                interactjsEvent = {
                    dx: 10,
                    dy: 20,
                    target: {
...
describe('create meme text', (): void => {
    ...
    beforeEach((): void => {
        ...
        inputColorPicker = document.querySelector('.new-meme-input-color-picker') as HTMLInputElement;
        inputFontSize = document.querySelector('.new-meme-input-font-size-number') as HTMLInputElement;
    });
    test('create meme text is successful', (): void => {
        ...
        const expectedParagraph = document.querySelector('#meme-preview-container div p') as HTMLParagraphElement;
        ...
        const expectedInputMemeText = document.querySelector('.new-meme-text-input') as HTMLInputElement;
        expect(Array.from(expectedInputMemeText.classList)).toEqual(['new-meme-text-input']);
        const expectedInputColorPicker = document.querySelector('.new-meme-input-color-picker') as HTMLInputElement;
        expect(expectedInputColorPicker.value).toBe('#ffffff');
        const expectedInputFontSizeNumber = document.querySelector('.new-meme-input-font-size-number') as HTMLInputElement;
        expect(expectedInputFontSizeNumber.value).toBe('20');
    });
    test('meme preview container is invalid', (): void => {
        let memePreviewContainer: HTMLDivElement = document.querySelector('#meme-preview-container') as HTMLDivElement;
