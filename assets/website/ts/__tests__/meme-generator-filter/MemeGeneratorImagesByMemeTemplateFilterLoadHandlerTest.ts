...
import MemeGeneratorImagesByMemeTemplateFilterLoadHandler
    from '../../src/meme-generator-filter/MemeGeneratorImagesByMemeTemplateFilterLoadHandler';
...
describe('Meme generator images by meme template filter loaded', (): void => {
    ...
    beforeEach((): void => {
        ...
        memeTemplateFilterButton = document.getElementById('xing-media-meme-template-filter-button') as HTMLAnchorElement;
    });
    ...
    test('Gallery images by gif filter loaded successful', (): void => {
        ...
        memeTemplateFilterButton = document.getElementById('xing-media-meme-template-filter-button') as HTMLAnchorElement;
        expect(Array.from(memeTemplateFilterButton.classList)).toEqual(['xing-media-filter-button-selected']);
    ...
    test('Meme generator images by no filter loaded successful', (): void => {
        ...
        memeTemplateFilterButton = document.getElementById('xing-media-meme-template-filter-button') as HTMLAnchorElement;
        expect(Array.from(memeTemplateFilterButton.classList)).toEqual(['xing-media-filter-button']);
    ...
    test('meme template filter button contains disabled class', (): void => {
        ...
        memeTemplateFilterButton = document.getElementById('xing-media-meme-template-filter-button') as HTMLAnchorElement;
        expect(Array.from(memeTemplateFilterButton.classList)).toEqual(['xing-media-filter-button-disabled']);
        expect(memeGeneratorImagesByMemeImageFilterLoaderMock.load).not.toHaveBeenCalled();
        expect(memeGeneratorImagesByNoFilterLoaderMock.load).not.toHaveBeenCalled();
        const expectedMediaContainer: Element = document.querySelector('.xing-media-container') as Element;
