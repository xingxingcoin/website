...
describe('Gallery images are manipulated', (): void => {
    ...
    test('Gallery images are manipulated successful', (): void => {
        ...
        let galleryImagesManipulator: GalleryImagesManipulator = new GalleryImagesManipulator(
            '.xing-media-container',
        );
        galleryImagesManipulator.displayImagesInGallery(jsonResponse);

        const expectedGalleryContainer: Element = document.querySelector(
            '.xing-media-container',
        ) as Element;
        expect(expectedGalleryContainer.innerHTML.replace(/\s+/g, '')).toBe(
            '<aclass="container-bottom-hidden"href="test"><divclass="xing-image-container"><imgsrc="test"alt="ImageofXing"></div></a><aclass="container-bottom-hidden"href="test2"><divclass="xing-image-container"><imgsrc="test2"alt="ImageofXing"></div></a>',
        );
    });
    test('Gallery images are manipulated successful', (): void => {
        let jsonResponse: MediaUrl[] = [
