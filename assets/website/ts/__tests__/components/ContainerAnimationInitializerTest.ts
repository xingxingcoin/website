...
describe('animate container', (): void => {
    ...
    test('animate container successfully', (): void => {
        let hiddenContainerLeft: HTMLElement = document.querySelector(
            '.container-left-hidden',
        ) as HTMLElement;
        let hiddenContainerBottom: HTMLElement = document.querySelector(
            '.container-bottom-hidden',
        ) as HTMLElement;
        jest.spyOn(hiddenContainerLeft, 'getBoundingClientRect').mockReturnValue({
            top: 500,
        } as DOMRect);
        ...
        const expectedHiddenContainerCollection: NodeList = document.querySelectorAll(
            '.container-left-hidden, .container-bottom-hidden',
        );
        expect((expectedHiddenContainerCollection[0] as HTMLElement).outerHTML).toBe(
            `<div class="container-left-hidden container-show"><button>Test</button></div>`,
        );
        expect((expectedHiddenContainerCollection[1] as HTMLElement).outerHTML).toBe(
            `<div class="container-bottom-hidden"><button>Test</button></div>`,
        );
    });
