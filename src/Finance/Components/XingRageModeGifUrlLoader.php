...
{
    public const string MEDIA_IMAGE_KEY = 'xing_information_image_rage_mode';
    ...
    {
        try {
            $this->logger->info('Start loading xing rage mode gif url.');
            ...
            $financeData = \array_merge($financeDataCollection->data, ['url' => $media->getFormats()['sulu-400x400'], 'mood' => 'Fired up and ready to explode.']);
            $this->logger->info('Xing rage mode gif url is loaded successfully.', [
                'mediaId' => $mediaId,
                'url' => $media->getFormats()['sulu-400x400'],
            ]);
        } catch (MediaNotFoundException $exception) {
            $this->logger->notice('Xing rage mode gif url is not loaded.');
            $this->logger->debug('Xing rage mode gif url is not loaded.', [
