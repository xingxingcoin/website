...
{
    public const string MEDIA_IMAGE_KEY = 'xing_information_image_calm';
    ...
    {
        try {
            $this->logger->info('Start loading xing calm mode gif url.');
            ...
            $financeData = \array_merge($financeDataCollection->data, ['url' => $media->getFormats()['sulu-400x400'], 'mood' => 'Peaceful and steady, just going with the flow.']);
            $this->logger->info('Xing calm mode gif url is loaded successfully.', [
                'mediaId' => $mediaId,
                'url' => $media->getFormats()['sulu-400x400'],
            ]);
        } catch (MediaNotFoundException $exception) {
            $this->logger->notice('Xing calm mode gif url is not loaded.');
            $this->logger->debug('Xing calm mode gif url is not loaded.', [
